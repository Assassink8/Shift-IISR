...
class UNetModelSwin(nn.Module):
    ...
    def forward(self, x, timesteps, lq=None, mask=None):
        ...
        out = self.out(h)
        return out
class UNetModelSwinAdagn(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def forward(self, x, timesteps, lq=None):
        ...
        for jj, module in enumerate(self.output_blocks):
            h = th.cat([h, hs.pop()], dim=1)
            h = module(h, emb)
            # print(f"output_blocks[{jj}]: {h.shape}")
        h = h.type(x.dtype)
        out = self.out(h)
...
class UNetModelSwinAdagnV2(nn.Module):
    def __init__(self,
                unet: nn.Module,
    ...
    def forward(self, x, timesteps, lq=None,
        ...
        if lq is not None:
            ...
            lq = self.unet.feature_extractor(lq.type(self.unet.dtype))
            x = th.cat([x, lq], dim=1)
